...
func printUsageAndExit(hint string) {
	...
	_, _ = fmt.Fprintf(os.Stderr, "NAME\n"+
		...
		"  uniq -D [-e] [-L] [-R] [<dir>]\n\n"+
		"DESCRIPTION\n"+
		"  uniq reads file paths from stdin and looks for duplicates by "+
		"computing the SHA1 checksum of each file. If <dir> is specified, "+
		"uniq evaluates files in <dir> (recursively if -R is "+
		"specified) instead.\n"+
		...
		"  After evaluating all files, uniq will exit with non-zero status "+
		...
		"uniq continues. This behavior may be changed by specifying -e, "+
		"which causes uniq to exit immediately if an error occurs. "+
		"Similarly, specifying -b causes uniq to exit immediately if a file "+
...
func main() {
	...
	if dir != "" {
		sums, err = uniq.FilterDir(dir, opts)
	} else {
		sums, err = uniq.Filter(os.Stdin, opts)
	}
